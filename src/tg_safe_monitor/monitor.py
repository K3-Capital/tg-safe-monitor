...
class SafeMonitorLoop:
    def __init__(self, service, *, send_message: Callable[[str], Awaitable[None]], poll_interval_seconds: int) -> None:
    ...
    async def run(self) -> None:
        while not self._stop_event.is_set():
            try:
                notifications = await self.service.poll_once()
                for notification in notifications:
                    await self.send_message(format_new_eoa_transaction_message(notification))
            except asyncio.CancelledError:
                raise
            except Exception:
                logger.exception("EOA monitor poll failed")
            try:
                await asyncio.wait_for(self._stop_event.wait(), timeout=self.poll_interval_seconds)
            except TimeoutError:
                continue
